...
from typing import List  # 兼容Python3.8及以下的类型注解
...
# 导入核心模块（保留原始导入，新增对话记忆实例）
from main import rag_pipeline, clear_conversation, conversation_memory
from generator.multi_turn import ConversationTurn  # 用于对话历史的类型提示
...
# 新增：显示对话历史函数（完全贴合需求，仅显示用户提问+最终答案）
def print_history(history: List[ConversationTurn], max_turns: int = 5):
    """显示最近的对话历史，默认最多5轮"""
    ...
    # 只显示最近的max_turns轮，避免输出过长
    display_turns = history[-max_turns:] if len(history) > max_turns else history

    for i, turn in enumerate(display_turns, 1):
        print(colored(f"\nTurn #{i}", "blue", attrs=["bold"]))
        print(colored(f"Q: {turn.question}", "cyan"))  # 用户提问
        # 截断过长的回答（保持界面整洁）
        answer = turn.answer[:200] + ("..." if len(turn.answer) > 200 else "")
        print(colored(f"A: {answer}", "green"))  # 最终答案
        print(colored("-" * 60, "gray"))

    if len(history) > max_turns:
        print(colored(f"\nShowing last {max_turns} turns (total {len(history)})", "blue"))
...
def print_help():
    ...
    print(colored("  /history        - Show recent conversation history (Q&A only)", "white"))  # 新增
...
def run_ui():
    ...
    try:
        ...
        print(colored(
            f"Warning during initialization: {e}\nYou can still continue, but the selected retriever may fall back to another.",
            "red"))
    ...
    while True:
        ...
        # 新增：处理/history命令
        if q.lower() == "/history":
            # 从全局对话记忆中获取历史（与clear_conversation对应）
            history = conversation_memory.turns
        ...
        # 处理正常提问（完全保留原始逻辑，不添加hybrid_alpha参数）
        if not q.strip():
            continue
        ...
        try:
            # 使用统一的 rag_pipeline 接口（保留原始hybrid特殊处理）
            if method == "hybrid":
                # 对于hybrid，使用 BM25 + instruction-dense 的混合检索
                from retrieval.hybrid import hybrid_bm25_instruction_retrieve
                docs = hybrid_bm25_instruction_retrieve(q, topk=10, alpha=hybrid_alpha, mode="score")
                ...
                # 非hybrid方法，仅传递q、method、mode（与原始一致，无hybrid_alpha）
                result = rag_pipeline(q, method=method, mode=mode)

            # -------------------------- 核心修复：补充必填参数 rewritten_query --------------------------
            # 多轮模式下用重写后的查询，基础模式下用原始问题（满足类构造函数要求）
            rewritten_query = result.get("rewritten_query", q)  # 优先取结果中的重写查询，没有则用原始问题
            current_answer = result.get("answer", "No answer provided.")
            # 传入所有必填参数：question（原始问题）、rewritten_query（重写查询）、answer（最终答案）
            new_conversation_turn = ConversationTurn(
                question=q,
                rewritten_query=rewritten_query,
                answer=current_answer
            )
            conversation_memory.turns.append(new_conversation_turn)
            # -----------------------------------------------------------------------------
